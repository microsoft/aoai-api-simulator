...
def set_simulator_translations_latency(endpoint: str, mean: float, std_dev: float):
    """
    Set the latency for the simulator completions endpoint

    :param endpoint: The simulator endpoint to set the latency for
    :param latency: The latency to set - specified in milliseconds per chat megabyte of input audio
    """
    ...
    response = requests.patch(
        url=url,
        headers={"api-key": api_key, "Content-Type": "application/json"},
        json={"latency": {"open_ai_translations": {"mean": mean, "std_dev": std_dev}}},
        timeout=10,
    )
    response.raise_for_status()
